...
| **Claims** | `claims.go` | JWT/API key validation; extracts role, rate, sessionID; `Rule` shape + `ExtractRules` helper for grace tokens. The one token parse (`Claims.Get`) behind `/`, `/session-stats/` and `/speedtest`. golang-jwt v4, not v5: v5 stops checking `iat` unless asked and changes the error texts `proxyHTTP` matches to log expired/malformed at Debug. `TestTokenParseSites` pins what each site accepts and refuses |
...
| **HybridBucketPool** | `hybrid_bucket.go` | Bandwidth limiter: a token bucket per (sessionID, rate), local tier + a Redis balance all pods share; see Bandwidth limiter |
...
### Bandwidth limiter (`hybrid_bucket.go`)

- Installed on external requests with `--use-bandwidth-limit` whose token carries `rate` and `sessionID`; none when either is missing (`HybridBucketPool.Get` returns nil). A `rate` bytefmt cannot parse (`5X`, `""`, a bare `5`) is not "no limiter": every request on the token fails with 500 and an Error `failed to get bucket` (`TestWebUnparsableRateFailsRequest`)
- One bucket per (sessionID, rate). `bucketKey` names it for both the pool's lazymap (5 min) and Redis: `bw:limit:<sessionID>:<rate in whole bytes/s>` (5M → `bw:limit:<sid>:655360`, 50M → `…:6553600`), a hash `{tokens, last_tick}` updated by one Lua script, `EXPIRE 300` on every call. Capacity = 1 s of the rate
- The rate is in the key because a session holds two at once: its tier's on the primary token (playlist polls, post-grace segments) and grace's 50M on grace segments. On one key (`bw:limit:<sessionID>`, until 2026-09-25) whichever drained the balance left the other nothing, and each call clamped it to its own capacity (`TestHybridBucketRatesOfOneSessionDoNotShareTokens`). So a session's limit is per rate: tier and grace requests open together can take both rates
- The rate is the one the script is told, not the claim's spelling: `5M` and `5MB` are one bucket (`TestHybridBucketSameRateSharedAcrossPods`). It holds no `:`, so the last `:` splits the key whatever the sessionID holds
- Old `bw:limit:<sessionID>` keys expire 5 min after their last write; nothing reads them (all webtor repos grepped 2026-09-25: the writer was the only reference)
- The rollout that brings this key (from an image keyed by session alone, e.g. sha-277778d): old and new pods keep separate balances for one session and rate, so a session whose requests reach both kinds can get up to twice its rate, once. None is limited twice, none goes unlimited. Not only the two pods of one node during its surge (`maxSurge: 1`): each infohash is served on its rendezvous home node, so a session on two torrents homed on nodes either side of the DaemonSet rollout draws on both keys from the first new pod until the last old pod has drained. 2026-09-25's rollout created its new pods at 20:39:15, :37 and :47Z: ~32 s, plus up to 70 s for the last old pod (10 s preStop + up to 60 s drain), ~100 s. Estimated from those timestamps, not measured. Check after it: `SCAN 0 MATCH bw:limit:*:*` in Dragonfly lists the new keys; old ones (a single `:` after `limit`) are gone 5 min after the last old pod
- Grace segment tokens carry no `sessionID` yet, so they get no bucket (see the blind spots under Observability). Once web-ui adds it, grace segments are limited at 50M in their own bucket, apart from the tier's, on pods running this key. Order: roll this key out to every node before web-ui puts `sessionID` in grace tokens; once it has, do not roll thp back past it (2026-09-25) without first rolling back web-ui's grace `sessionID`. An older pod keys by session alone: grace (50M) and tier (5M) calls share one balance and clamp it to their own capacity, silently (`throttle_wait_seconds_total{role="grace"}` shows grace waiting, not why). Such a rollback also drops `acquireGrace`: session stats' `rate` flips between 5M and 50M again and grace waits read as the tier's (see Observability)
- Redis down: each pod falls back to local accrual and probes Redis every 5 s
...
- 499 (`StatusClientClosedRequest`, nginx's convention; class `400`): the client's own context is done (captured at the top of `proxyHTTP`, before anything derives it) and the error is `context.Canceled`. Logged Info `client closed request`. Recorded only (metrics, closing line): `errorHandler` still writes the 502 it always wrote, because a done context is not a gone reader — net/http cancels on EOF from the client, and a client that half-closed (FIN on its write side) still reads the answer (`TestWebClientHalfClosedStillGets502`)
...
- A long `duration` on a 499 is a client that gave up waiting for headers, not one that changed its mind: 2,036 of the seeder's 4,131 external ones ended at 30 s or 60 s (client timeouts on a seeder that had not answered: ingress-nginx logged its own 499 for all 30 of them sampled by `req_id`, so it was the end client, not an ingress timeout answering 504). Upstream stalls now show as 499s with long durations, not as 5xx
...
- JSON: `window_sec` (5); `bytes_per_sec` over the last window (over the stream's age while younger: events 2–5 cover 1–4 s; the first spans nothing: 0 and no `throttled`, speed unknown until the second); `conns` open content requests now (a gauge read at the tick); `active` (always sent, see below); `rate` claim of the latest request not on a grace token, absent when none; `throttled` = Δ limiter wait of all the key's requests but grace segments, summed / the same wall-clock span, clamped to 0..1, absent when no limited request was open in the window (no limiter ≠ 0; the open-time integral `limitedOpen` only answers that)
- `active` = `conns > 0` at this sample, or the entry's `ends` (requests ended, ever; one atomic add in `release`, none on `Write`) moved since this stream's previous sample; the first event is `conns > 0` alone. Like `conns`, it counts a request from its final `WriteHeader` or first `Write` (see "Counted"): a request waiting for its first byte (content-transcoder's `WaitForSegment` before any header, after a seek or when transcoding lags playback) is in neither, so a viewer whose only request is such a wait reads `active:false` (`TestSessionStatsStreamSkipsRequestAwaitingFirstByte`). Do not acquire earlier to cover it: before the final status is known, 404s for made-up hashes would fill the map. Mitigation, not a guarantee: while FFmpeg is producing, the transcoder serves the playlist as `EVENT` without `ENDLIST`, so an HLS player keeps reloading it during the wait, and each reload is a short 2xx content request. Why: a paid viewer (no limiter) gets each HLS segment from the transcoder in well under a second, so the 1 Hz `conns` gauge read 0 in every event while `bytes_per_sec` showed 1.1–2.2 MB/s, and web-ui never drew the viewer (2026-09-25). Opens need no counter: open at the previous sample and still open, or opened since and still open, is `conns`; everything else that was open in between has ended. A counter, not a flag the reader clears (the first of up to 4 streams on the key would take it from the others: `TestSessionStatsActiveSeenByEveryStream`), not `lastActive` (two ends in one clock reading look like none; the fake clock does exactly that). `ends` is in `statsTotals`, so `push` zeroes it with the rest on an entry change: a new entry reads active. `release` adds to `ends` before `conns` falls and `sample` loads `conns` first, so a sample that no longer counts a request open sees it ended; no test pins that order (a ns window; reversed, a request reads active one event later)
- `active` includes web-ui's own server-side fetches on the viewer's token (see "Also counted"): a probe or subtitle fetch reads active for that second. web-ui's presence rule: `active` when present, else `conns > 0 || bytes_per_sec > 0` (older proxy), with its 2 s debounce
...
- Grace segments once their token carries `sessionID` (`isGraceToken`: `kind` grace): `acquireGrace` counts their bytes, `conns` and `active` (the viewer's), not their `rate` or their limiter's wait (the grace window's). `rate` and `throttled` stay the tier's however the key's requests interleave: otherwise every playlist poll on the primary token between two grace segments would flip `rate` between 5M and 50M, and a binding grace bucket would read as the tier binding, a CTA inside grace (`TestSessionStatsGraceSegmentKeepsTierRateAndWait`). So in grace `bytes_per_sec` can exceed 1 s of `rate` with `throttled` ≈ 0, and up to `window_sec` after it ends the window still holds grace bytes. An entry whose only requests so far are grace segments has no `rate` (the playlist on the primary token normally comes first). The grace token must also carry the primary's `domain`: without it an embed's grace bytes land on the owner's `default` key. Chosen 2026-09-26, before web-ui adds `sessionID`, for web-ui's rule "no CTA inside grace: it is not the tier binding" and its post-grace CTA against the tier's ceiling; behaviour-neutral until then (no grace request has a session, so none reaches the writer)
