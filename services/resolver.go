...
func (s *Resolver) process(i *Init, logger *logrus.Entry, purge bool) (*Location, error) {
	if i.ConnectionConfig.ConnectionType == ConnectionType_SERVICE {
		...
		return s.jobLocPool.Get(&i.ConnectionConfig.JobConfig, i.InitParams, logger, purge)
	}
}

func (s *Resolver) Resolve(src *Source, logger *logrus.Entry, purge bool) (*Location, error) {
	logger = logger.WithField("purge", purge)
	init := s.getInit(src)
	l, err := s.process(init, logger, purge)
	if err != nil {
